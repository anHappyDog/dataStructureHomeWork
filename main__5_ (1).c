...
{
	pNode tempPtrl = NULL;
	unsigned int kbd2 = 0, kbd3 = 0;
	int len = 0, webpage = 1, firstNum = 0, lastNum = 0, preLoc = -1,index = 0;
		...
		for (int i = 0; i < len; i++)
		{
			...
			else if (!isalpha(buf[i]) && preLoc != -1)
			{
				...
				if (index == 1)
				{
					...
					if (index == 1)
					{
						tempPtrl->keyWordCnt[kbd3]++;
						if (tempPtrl->keyWordCnt[kbd3] == 1)
							keyShowCount[kbd3]++;
					}
				}
